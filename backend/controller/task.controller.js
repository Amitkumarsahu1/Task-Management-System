...
export const getTasks = async (req, res, next) => {
  try {
    ...
    const totalTasksForFilter = await Task.countDocuments(
      req.user.role === "admin" ? filter : { ...filter, assignedTo: req.user.id }
    )

    const totalPages = Math.ceil(totalTasksForFilter / limitNum)
    ...
    res.status(200).json({
      ...
      pagination: {
        currentPage: pageNum,
        totalPages,
        totalTasks: totalTasksForFilter,
